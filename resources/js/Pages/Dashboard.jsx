...
export default function Dashboard({ count, page_settings, productivity_chart, tasks }) {
    ...
    const data = {
        labels: productivity_chart.labels,
            ...
            <aside className="fixed inset-y-0 right-0 hidden w-96 overflow-y-auto border-l border-gray-200 px-4 py-6 lg:px-8 xl:block">
                <Card className="mt-4">
                    <CardContent className="flex flex-col items-center">
                        <Avatar className="my-2 h-14 w-14">
                ...
                <Card className="mt-4">
                    <CardHeader>
                        <CardTitle className="text-lg leading-relaxed">Your Tasks</CardTitle>
                    </CardHeader>
                    <CardContent>
                        {tasks.map((task, index) => (
                            <Link key={index} href={task.memberable.detail} className="hover:text-blue-500">
                                <div className="mb-4 flex items-center gap-x-2">
                                    <div className="rounded-full bg-blue-500 p-3">
                                        <PiNewspaperClipping className="h-6 w-5 text-white" />
                                    </div>
